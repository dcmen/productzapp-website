...
Vcore.Customer = function(){
    $(document).on('click', '.del_network', function () {
        id = $(this).attr("user-id");
        item = $(this);
        jConfirm('Are you sure want to delete this user?','Message',function(r) {
            if(r){
                load_show();
                $.post(root + 'network_del',{'id':id},function(data){
                    if(data.error == 0){
                        // update number dealer, hide table if no dealer
                        ...
                        if (countDealer == 0) {
                            ...
                            $('#NetWorkPage').append('<div class="mg-top-50 text-center font-size-24"><span>No dealer to display</span></div>');
                            load_hide();
                        }
                ...
                $.post(root + 'blockuser',{id:block_id}, function(data){
                    if(data.error == 0){
                        ...
                        item.closest('tr').fadeOut('slow', function() {
                            ...
                            // show message and refresh page
                            if (countDealer > 0 && $('tbody > tr').length === 0) {
                                showMessage('Blocked successfully', 0, function () {
                                    curPage = (curPage < maxPage)? curPage : --curPage;
                                    curPage = (curPage < 1)? 1 : curPage;
                                    maxPage--;
                ...
                if(r){
                    ...
                    $.post(root + 'customers/delcustomer',{'str_id':str_id},function(data){
                        ...
                        if (data.error == 0) {
                            ...
                            // delete all row has checked
                            $(".customer input[type='checkbox']").each(function(){
                                if($(this).is(":checked") && $(this).attr("id") != "checkall" && !$(this).is(':disabled')){
                                    $(this).closest('tr').fadeOut('slow', function() {
                                        ...
                                        if ($('tbody > tr').length === 0) {
                                            ...
                                            $('.total-count').text($('tbody > tr').length);
                                        }
                                    });
                                }
                            });
    ...
    $(document).on('click', '.un_blockuser', function (){
        block_id = $(this).attr('user-id');
        item = $(this);
           ...
           if(r){
                ...
                $.post(root + 'unblockuser',{id:block_id}, function(data){
                    if(data.error == 0){
                        ...
                        if (countDealer == 0) {
                            $('.network-content').hide();
                            $('#BlockNetWorkPage').append('<div class="mg-top-50 text-center font-size-24"><span>No dealer to display</span></div>');
                            load_hide();
                        }
                        // remove row contain dealer
                        item.closest('tr').fadeOut('slow', function() {
                            $(this).remove();
                            // show message and refresh page
                            showMessage('Unblocked successfully', 0, function () {
                                if (countDealer > 0 && $('tbody > tr').length === 0) {
                                    curPage = (curPage < maxPage)? curPage : --curPage;
                                    curPage = (curPage < 1)? 1 : curPage;
                                    maxPage--;
                                    
                                    loadHTMLAjax('block_network', $.extend(parasOnLink, {page:curPage, ajax:1}), container = '#BlockTable');
                                }
                            });
                        });
...
Vcore.Addnetwork = function(){
    $(document).on('click', '.btn-resend-invite', function () {
        ...
        sender_name = $(this).attr('data-sender-name');
        request_email = $(this).attr('data-request-email');
        
        load_show();
        $.post(root + 'InviteNetworks/resendInviteNetwork', {invite_id:invite_id, sender_id:sender_id, sender_name:sender_name, request_email:request_email},function(data){
            if(data.error == 0){
                showMessage('Your request has been resent successfully',0);
            }else{
                showMessage('Failure',1);
            }
            load_hide();
    ...
    $(document).ready(function(){
        $(document).on('click', '.clickfollow', function() {    
            ...
            if (element.hasClass('dis_follow')) {
                load_show();
                $.post(root + 'cars/follow', {'car_id':car_id,'user_id':userId,'type':1},function(data){
                    if(data.error == 1){
                        showMessage('Failure', data.error);
                        load_hide();
                    }else{
                        // if in flicka page
                        $('.car-detail-01#' + car_id).removeClass('dis_follow');
                        // change color of star
                        element.removeClass('dis_follow');
                        element.addClass('follow');
                        star = element.find('.fa');
                        star.removeClass('star-yellow');
                        star.attr('title', 'Follow this car');
                        // if in followed cars page
                        if ($('#FollowPage').length > 0) {
                            // update number followed cars on menu bar
                            countFollowed = parseInt($('.count-followed').text());
                            if (countFollowed > 0) {
                                $('.count-followed').text(--countFollowed);
                            }
                            if (countFollowed == 0) {
                                $('#flickaViewList').hide();
                                $('#FollowPage').append('<div class="mg-top-50 text-center font-size-24"><span>No cars to display</span></div>');
                                load_hide();
                            }
                            // hide item (in follow page)
                            $('#FollowPage #' + car_id).fadeOut('slow', function() {
                                $(this).remove();
                                // if current page has no car => load new page data
                                if (countFollowed > 0 && $('#FollowPage .wg-car-list').length === 0) {
                                    // set current page
                                    curPage = (curPage < maxPage)? curPage : --curPage;
                                    curPage = (curPage < 1)? 1 : curPage;
                                    // update max page
                                    maxPage--;
                                    // load ajax
                                    loadHTMLAjax('followed', $.extend(parasOnLink, {page:curPage, ajax:1}), container = '#listCars');
                                }
                                else {
                                    load_hide();
                                }
                            ...
                            //showMessage('Unfollowed successfully', 0);
                            load_hide();
                        }
                    }
                },'json');
                ...
                load_show();
                $.post(root + 'cars/follow', {'car_id':car_id,'user_id':userId,'type':0},function(data){
                    load_hide();
                    if(data.error == 1){
                        showMessage('Failure', data.error);
                    }else{
                        // if in flicka page
                        $('.car-detail-01#' + car_id).addClass('dis_follow');
                        ...
                        //showMessage('Followed successfully', 0);

                        starElement = element.find('.fa');
                        starElement.addClass('star-yellow');
                        starElement.attr('title', 'Unfollow this car');
                    }
                },'json');
            }
        });
...
//            
//            if (element.hasClass('dis_follow')) {
//                jConfirm('Do you want to unfollow this car?','Messages',function(r){
//                    if(r){
//                        load_show();
//                        $.post(root + 'cars/follow', {'car_id':car_id,'user_id':userId,'type':1},function(data){
//                            if(data.error == 1){
...
//                                        $(this).remove();
//                                        // show message and refresh page
//                                        showMessage('Unfollowed successfully', 0, function () {
//                                            // if current page has no car => load new page data
//                                            if (countFollowed > 0 && $('#FollowPage .wg-car-list').length === 0) {
//                                                // set current page
//                                                curPage = (curPage < maxPage)? curPage : --curPage;
...
//            }
//            else {
//                jConfirm('Do you want to follow this car?','Messages',function(r){
//                    if(r){
//                        load_show();
//                        $.post(root + 'cars/follow', {'car_id':car_id,'user_id':userId,'type':0},function(data){
//                            load_hide();
    ...
    $(document).ready(function() {
        ...
        $(".choosemake").click(function(){
            ...
            if(make == ''){
                ...
                window.location.href = root +'flicka?type=Make&make='+make;
            }
        });
        ...
        $(".chooseyear").click(function(){
            ...
            if (year_to >= year_from) {
                ...
                showMessage('Year (from) must be less than (to)', 1);
            }
        });
...
Vcore.Flicka.CarsforSale = function(){
    ...
    $(".chosen-select").chosen({no_results_text: "Not found!", width: "100%"});
...
//    });
    ...
    $(".changemake-car4sale").change(function(){
        load_show();
        make = $(this).val();
        $.post(root + 'cars/changemake_car4sale',{'make':make}, function(data){
            $("#boxModel").html(data);
            $("#boxModel").trigger("chosen:updated");
    ...
    $(".changemodel-car4sale").change(function(){
        load_show();
        model = $(this).val();
        make = $("select[name='make']").val();

        $.post(root + 'cars/changemodel_car4sale',{'make':make,'model':model}, function(data){
            $("#boxSeries").html(data);
            $("#boxSeries").trigger("chosen:updated");
            load_hide();
        });
                    ...
                    if(r){
                        ...
                        $.post(root + 'Cars/ajaxchangecarprice',{car_id:carId, price:price, type:1},function(data){
                            if (data.error == 0) {
                                ...
                                showMessage('Changed successfully', 0);
                            }
                            else {
                                showMessage(data.msg, 1);
                            }
...
$(document).on('click', '.wg-car-btnvideo', function () {
    urlVideo = $(this).attr('data-video-url');
    var myId = getIdYoutube(urlVideo);
    $('#viewVideoModal iframe').attr('src', 'https://www.youtube.com/embed/' + myId);
    $('#viewVideoModal').modal('show');
});

$(document).on('click', '.btn-post-report', function () {
    postId = $(this).attr('data-post-id');
    $('#PostRepostModal #PostReportForm .car-id').val(postId);
    $('#PostRepostModal #PostReportForm').attr('action', root + 'report_pulse/' + postId);
    $('#PostRepostModal').modal('show');
});

$(document).on('click', '.btn-logout', function () {
    jConfirm('Are you sure you want to logout?', 'Carzapp', function(r){
        if(r){
            localStorage.clear();
            load_show();
...
$(document).on('click', '#cz-kb-alert-message .close_message', function () {
...
function showMessageold($message, $type, callback) {
...
function showMessage($message,$type,callback){
    $('.kb-alert-message-popup').attr('data-type',$type);
    $('.kb-alert-message-popup .kb-alert-message-content .kb-alert-message-body').html($message);
    $('.kb-alert-message-popup').css('display','block');
    // set event
    $('#cz-kb-alert-message .close').unbind('click');
    $('#cz-kb-alert-message .close').bind('click', function () {
        if ( typeof callback == 'function' ) {
            callback();
        }
        $('.kb-alert-message-popup').css('display', 'none');
...
function setCurrentDate(target) {
    // format dd-mm-yyyy
    var date = new Date();
...
function loadHTMLAjax(urlAjax, params, container, scrollTop) {
    load_show();
    $.get(urlAjax, params , function(data) {
        $(container).html(data);
        if (scrollTop) {
...
function initPagination(totalPage, page, maxVisible, url, container) {
    if (totalPage > 1) {
        $('.cz-pagination').bootpag({
            ...
            curPage = num;
            loadHTMLAjax(url, $.extend(parasOnLink, {page:num, ajax:1}), container, scrollTop = true);
        });
    }
    else {
        $('.cz-pagination').html('');
...
function updatePagination(totalPage, page, maxVisible, url, container) {
    if (totalPage > 1) {
        $('.cz-pagination').bootpag({
            total: totalPage,
            page: page,
...
function showBGCoverTransparent() {
    $('.bg-cover-transparent').show();
}

function hideBGCoverTransparent() {
    $('.bg-cover-transparent').hide();
}
