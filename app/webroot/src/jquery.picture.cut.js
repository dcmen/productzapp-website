    ...
    var methods = {
        ...
        init: function(Options) {
            ...
            if ($("#" + basic_dependence_css_id).size() == 0) {
                $('<style type="text/css" id="' + basic_dependence_css_id + '">' + ".picture-element-principal{background:#002d56 url(" + Options.imgAvata + ") no-repeat 50% 50%; background-size: cover;left:0;top:0;border-radius: 50%;}" + ".picture-dropped{border:2px #666 dashed!important;}" + '</style>').appendTo('head');
            }
            return this.each(function() {
                var Elemento;
                var CropWindowStyle = {
                ...
                var TuyoshiCrop = function(element, response) {
                    ...
                    var MontarSelecaoRecorte = function(SelecaoRecorte, state) {
                        ...
                        ElemSelectProporcao.parent().removeClass('col-xs-2');
                        ElemSelectProporcao.parent().addClass('col-md-3');
                        ElemSelectOrientacao.parent().removeClass('col-xs-2');
                        ElemSelectOrientacao.parent().addClass('col-md-3');
                        if (state == "create") {                            
                            if (Options.CropModes.widescreen || Options.CropModes.letterbox || Options.CropModes.free  || Options.CropModes.square) {
                            ...
                            if (Options.CropModes.square) ElemSelectProporcao.append($('<option value="square">1:1</option>'));
                            ...
                            if (ElemSelectProporcao.val() == "livre") {
                                ...
                                var InitRatio = 0;
                                if (ElemSelectProporcao.val() == "square") {
                                    InitRatio = 1;
                                    if (response.currentHeight > response.currentWidth) {
                                        Swidth = (response.currentWidth / 100) * 80;
                                        Sheight = Swidth;
                                    }
                                    else {
                                        Swidth = (response.currentHeight / 100) * 80;
                                        Sheight = Swidth;
                                    }
                                }
                                else if (ElemSelectProporcao.val() == "wide") {
                                    InitRatio = 16 / 9;
                                    Swidth = (response.currentWidth / 100) * 80;
                                    Sheight = (Swidth / 16) * 9
                                } else {
                    ...
                    var JpaneDialogCallBack = function() {
                        ...
                        $("#JtuyoshiCrop #button_crop_original").parent().parent().removeClass('col-md-8');
                        $("#JtuyoshiCrop #button_crop_original").parent().parent().addClass('col-md-6');
                        $("#JtuyoshiCrop #button_crop_original").bind("click", function() {
                            Retorno_Requisicao(element, response, 1);
                            $(".div_upload i").removeClass('fa-user');
                        });
