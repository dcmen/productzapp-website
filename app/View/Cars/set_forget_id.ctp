    ...
    <div class="pd-content-01">
        <div class="col-lg-12">
            <div class="wg-box-shadow pd-content-06">
                <form id="SetandforgetSetForgetIdForm" action="<?php echo $this->Html->Url('/set_forget_id/' . $id)?>" method="post">
                    ...
                    <div class="col-lg-12 form-item-container">
                        <div>
                            ...
                            <select name="customer_id" class="chosen-select form-control" data-placeholder="Choose a Customer...">
                                <?php foreach($customers as $rs):?>
                                <option value="<?php echo $rs->_id?>" <?php echo ($setfirst == $rs->_id)? 'selected="selected"': ''?>>
                    ...
                    <div class="col-lg-12 form-item-container">
                        <div>
                            ...
                            <a class="form-control kb-input-item" href="javascript:;" data-toggle="modal" data-target="#mynetwork" rel="mynetwork">
                                <label style="cursor: pointer;" class="msg-number-shared">Shared <?php echo sizeof($arr_share_dealer) ?> dealer(s)</label>
                            </a>
                        </div>
                    </div>
                    <div class="clearfix"></div>
                    ...
                    <div class="col-lg-12 form-item-container">
                        <div class="form-group">
                            ...
                            <div class="pos-rel">
                                <input type="text" name="keyword" class="form-control kb-input-item" value="<?php echo (isset($rel->keyword))?$rel->keyword:''?>" placeholder="Keyword: eg: Rego, body, make, features">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-6 form-item-container">
                        <div class="form-group">
                            ...
                            <select name="manu_year_from" class="chosen-select form-control changeyearsearch">
                                ...
                                <option <?php echo (isset($rel->manu_year_from) && $rel->manu_year_from == $manu_year_from[$i])?'selected="selected"':''?> value="<?php echo $manu_year_from[$i]?>"><?php echo $manu_year_from[$i]?></option>
                                <?php }?>
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-6 form-item-container">
                        <div class="form-group">
                            ...
                            <select name="manu_year_to" class="chosen-select form-control changeyearsearch">
                                ...
                                <option <?php echo (isset($rel->manu_year_to) && $rel->manu_year_to == $manu_year_to[$i])?'selected="selected"':''?> value="<?php echo $manu_year_to[$i]?>"><?php echo $manu_year_to[$i]?></option>
                                <?php }?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-6 form-item-container">
                        <div class="form-group">
                            ...
                            <select id="boxMake" name="make" class="chosen-select form-control changemakesearch">
                                <option value="">Any</option>
                                <?php foreach($makes as $data) {
                                    echo '<option '.(($data->code == $rel->make)? 'selected' : '').' data-code="' . $data->code . '" value="' . $data->name . '">' . $data->name . '</option>';
                                } ?>
                            </select>
                    ...
                    <div class="col-lg-6 form-item-container">
                        <div class="form-group">
                            ...
                            <select id="boxModel" name="model" class="chosen-select form-control changemodelsearch">
                                <option value="">Any</option>
                                <?php foreach($models as $data) {
                                    echo '<option '.(($data->code == $rel->model)? 'selected' : '').' data-code="' . $data->code . '" value="' . $data->name . '">' . $data->name . '</option>';
                                } ?>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="variant" id="boxVariant" class="chosen-select form-control changevariantsearch">
                                ...
                                <?php foreach($variants as $data) {
                                    echo '<option '.(($data->name == $rel->variant)? 'selected' : '').' value="' . $data->name . '">' . $data->name . '</option>';
                                } ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="series" id="boxSeries" class="chosen-select form-control">
                                ...
                                <?php foreach($series as $data) {
                                    echo '<option '.(($data->code == $rel->series)? 'selected' : '').' data-code="' . $data->code . '" value="' . $data->name . '">' . $data->name . '</option>';
                                } ?>
                            </select>
                    ...
                    <div class="col-lg-6 form-item-container">
                        <div class="form-group">
                            ...
                            <select name="price_from" class="chosen-select form-control">
                                ...
                                <option <?php echo (isset($rel->price_from) && str_replace("$","", str_replace(",","", $rel->price_from)) == str_replace("$","", str_replace(",","", $price_from[$i])))?'selected="selected"':''?> value="<?php echo $price_from[$i]?>"><?php echo $price_from[$i] ?></option>
                                <?php }?>
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-6 form-item-container">
                        <div class="form-group">
                            <label class="txt-lb-name dis-none-max-640">Price (to)</label>
                            <select name="price_to" class="chosen-select form-control">
                                <option value="">Any</option>
                                <?php for($i=0;$i<sizeof($price_to);$i++){?>
                                <option <?php echo (isset($rel->price_to) && str_replace("$","", str_replace(",","", $rel->price_to)) == str_replace("$","", str_replace(",","", $price_to[$i])))?'selected="selected"':''?> value="<?php echo $price_to[$i]?>"><?php echo $price_to[$i] ?></option>
                                <?php }?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-12 mg-top-10">
                        <button type="submit" class="kb-btn-02 color-bg-site pull-right save_set_forget">SAVE<span class="fa fa-angle-right"></span></button>
                    </div>
                    <div class="clearfix"></div>
                </form>
                <div class="clearfix"></div>
                ...
                <div class="modal-body">
                    <div class="content-make">
                        ...
                        <div class="line-form">
                            ...
                            <input type="checkbox" class="c_make" value="<?php echo $mk->_id?>" <?php echo (in_array($mk->_id, $arr_share_dealer))?'checked="checked"':''?>>
                            <?php echo $mk->name?>
                            </label>
                        </div>
                        <?php 
...
<script type="text/javascript">
    ...
    var listMake = <?php 
                        ...
                        foreach($makes as $data) {
                            $listMake[$data->name] = $data->code;
                        } 
                        echo json_encode($listMake);
                    ?>;
    var listModel = <?php 
                        $listModel = array();
                        foreach($models as $data) {
                            $listModel[$data->name] = $data->code;
                        } 
                        echo json_encode($listModel);
                    ?>;
    var listSeries = <?php 
                        $listSeries = array();
                        if ($series) {
                            foreach($series as $data) {
                                $listSeries[$data->name] = $data->code;
                            }
                        }
                        echo json_encode('');
                    ?>;
    ...
    $(document).ready(function(){
        ...
        $(".check_my_network").click(function(){
            ...
            $('.msg-number-shared').html('Shared '+count+' dealer(s) from "My Network"');
            if(str_id != ''){
                $("#SetandforgetSetForgetIdForm input[name='arr_mynetwork']").val(str_id);
            }else{
                $("#SetandforgetSetForgetIdForm input[name='arr_mynetwork']").val('');
            }
        ...
        $(".save_set_forget").click(function() {
            ...
            $('#SetandforgetSetForgetIdForm').submit();
